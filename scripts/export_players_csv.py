...
(or `line`) directly in the sheet and pull changes back in with
import_players_csv.py. See README.md's "Editing player ratings" section.
...
def main() -> None:
    ...
    writer.writerow(["id", "name", "team", "era", "pos", "ovr", "line"])
    for i, m in enumerate(ROW.finditer(text)):
        name, team, era, pos, ovr, line = m.groups()
        writer.writerow([i, unesc(name), unesc(team), era, pos, ovr, unesc(line)])
